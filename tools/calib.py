...
class CameraCalibrator:
    def __init__(self, pattern_size=(9, 6), square_size=30, min_samples=15, save_images=True):
        ...
        self.objp[:, :2] = np.mgrid[0:self.pattern_size[0],
                                    0:self.pattern_size[1]].T.reshape(-1, 2) * square_size
        ...
        while sample_count < self.min_samples:
            ...
            if success:
                cv2.drawChessboardCorners(
                    draw_frame, self.pattern_size, corners, success)
    ...
    def perform_calibration(self, cap=None, img_size=None):
        ...
        for i in range(len(self.obj_points)):
            ...
            error = cv2.norm(
                self.img_points[i], imgpoints2, cv2.NORM_L2) / len(imgpoints2)
            mean_error += error
        ...
        # 基于焦距和高度的像素实际尺寸换算
        self.pixel_ratio = self.square_size / \
            (fx * self.square_size / (self.height * 1000))
    ...
    def save_calibration_to_config(self, config_filename='../config/config car.json'):
        """将标定结果保存到config car.json文件的camera_down_correct中"""
        if not self.calibration_done:
            print("尚未完成标定，无法保存结果")
            return False
        ...
        try:
            ...
            if os.path.exists(config_filename):
                ...
                print(f"配置文件 {config_filename} 不存在")
                return False

            # 更新camera_down_correct部分
            if 'camera' not in config:
                config['camera'] = {}

            config['camera']['camera_down_correct'] = calibration_data

            # 保存回配置文件
            with open(config_filename, 'w', encoding='utf-8') as f:
                json.dump(config, f, indent=4, ensure_ascii=False)

            print(f"标定结果已保存到 {config_filename} 的 camera_down_correct 中")
            return True
        except Exception as e:
            print(f"保存标定结果失败: {e}")
            return False

    def measure_pixel_ratio(self, cap):
        """测量当前的像素与物理距离比值"""
        print("=== 像素比例测量模式 ===")
        print(f"请将 {self.pattern_size[0]}x{self.pattern_size[1]} 的棋盘格放在摄像头前")
        print(f"棋盘格方格边长: {self.square_size}mm")
        print("按空格键测量，ESC退出")
        ...
        while True:
            ...
            if success:
                ...
                for i in range(len(corners) - 1):
                    if (i + 1) % self.pattern_size[0] != 0:  # 同一行的相邻点
                        dist = np.linalg.norm(corners[i+1] - corners[i])
                        corner_distances.append(dist)

                if corner_distances:
                    avg_pixel_dist = np.mean(corner_distances)
                    pixel_ratio = self.square_size / avg_pixel_dist

                    cv2.putText(draw_frame, f"Pixel ratio: {pixel_ratio:.4f} mm/pixel",
                                (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                    cv2.putText(draw_frame, f"Avg pixel dist: {avg_pixel_dist:.2f} px",
                                (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                    cv2.putText(draw_frame, "Press SPACE to measure",
                                (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
            else:
                cv2.putText(draw_frame, "No chessboard detected", (20, 40),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                cv2.putText(draw_frame, "Adjust chessboard position", (20, 70),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

            cv2.imshow("Pixel Ratio Measurement", draw_frame)
            ...
            if key == 32 and success:  # 空格键
                ...
                print("\n=== 像素比例测量结果 ===")
                print(f"棋盘格方格边长: {self.square_size} mm")
                print(f"平均像素距离: {avg_pixel_dist:.2f} 像素")
                print(f"像素比例: {pixel_ratio:.4f} mm/像素")
                print(f"反比值: {1/pixel_ratio:.4f} 像素/mm")
                print("=" * 30)
                break
            elif key == 27:  # ESC键
                print("测量取消")
...
def load_calibration_from_config(config_filename='../config/config car.json'):
    """从config car.json文件的camera_down_correct中加载标定结果"""
    ...
    try:
        ...
        if 'camera' not in config or 'camera_down_correct' not in config['camera']:
            print("配置文件中未找到camera_down_correct数据")
            return None

        data = config['camera']['camera_down_correct']

        # 检查是否有标定数据
        if not data or 'camera_matrix' not in data:
            print("camera_down_correct中未找到有效的标定数据")
            return None
...
def undistort_and_display(config_filename='../config/config car.json', camera_id=0, apply_roi_crop=False):
    """从配置文件读取参数进行实时畸变矫正显示"""
    # 加载标定数据
    calib_data = load_calibration_from_config(config_filename)
    ...
    while True:
        ...
        if key == 27:  # ESC键
            ...
            if undistorted.shape[:2] != frame.shape[:2]:
                # 如果尺寸不同，调整矫正图像尺寸
                undistorted_resized = cv2.resize(
                    undistorted, (frame.shape[1], frame.shape[0]))
            else:
            ...
            cv2.putText(comparison, "Corrected",
                        (frame.shape[1] + 20, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
if __name__ == "__main__":
    # 定义参数
    config_file = './config/config car.json'
    camera_id = 2

    # 检查是否要加载现有标定文件
    calib_data = load_calibration_from_config(config_file)
    if calib_data is not None:
        print(f"检测到配置文件中的标定数据")
        choice = input("选择操作: [r]重新标定, [u]使用现有标定显示矫正结果, [m]测量像素比例? ").lower()
        if choice == 'u':
            undistort_and_display(config_file, camera_id)
            exit(0)
        elif choice == 'm':
            # 初始化标定器进行像素比例测量
            calibrator = CameraCalibrator(square_size=20, pattern_size=(5, 4))
            ...
            if cap.isOpened():
                ...
                calibrator.measure_pixel_ratio(cap)
                cap.release()
            else:
                print("无法打开摄像头")
            exit(0)
        ...
        # 没有现有标定数据时的选择
        choice = input("选择操作: [c]相机标定, [m]测量像素比例? ").lower()
        if choice == 'm':
            # 初始化标定器进行像素比例测量
            calibrator = CameraCalibrator(square_size=20, pattern_size=(5, 4))
    ...
    calibrator = CameraCalibrator(
        square_size=20, min_samples=50, pattern_size=(5, 4), save_images=True)
    ...
    if calibrator.collect_samples(cap):
        ...
        if calibrator.perform_calibration(cap=cap):
            ...
            # 保存标定结果到配置文件
            if calibrator.save_calibration_to_config(config_file):
                # 关闭摄像头
                cap.release()
